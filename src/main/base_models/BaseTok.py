...
class BaseTok(torch.nn.Module):
    def __init__(self, config):
        """
        param config: config object
        """
        super(BaseTok, self).__init__()
        self.config = config
    ...
    def forward(self, batch_tk):
        ...
        for (idx, vocab) in self.dict_vocab.items():
            qry_lkup, pos_lkup, neg_lkup = get_qry_pos_neg_tok_lookup(vocab, qry_tk[:,idx], pos_tk[:,idx], neg_tk[:,idx])
            batch_tk.append((qry_lkup, pos_lkup, neg_lkup))
    ...
    def evaluate(self, batch_tk):
        ...
        batch_tk = []
        for (idx, vocab) in self.dict_vocab.items():
            qry_lkup, cnd_lkup = get_qry_cnd_tok_lookup(vocab, qry_tk[:,idx], cnd_tk[:,idx])
            batch_tk.append((qry_lkup, cnd_lkup))
