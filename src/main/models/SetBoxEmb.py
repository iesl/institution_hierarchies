...
from main.base_models.BoxesEmb import BoxesEmb
...
class SetBoxEmb(BoxesEmb):
    def __init__(self, config, data_extractor, tokenizer):
        ...
        """
        super(SetBoxEmb, self).__init__(config, data_extractor, tokenizer)

        self.emb_counter = 0
        self.inst_emb = BoxEmb(self.dict_vocab[self.emb_counter].size + 1, self.config.inst_emb_dim)
        self.list_emb = [self.inst_emb]
        self.emb_counter += 1

        if self.config.include_city:
            self.city_emb = BoxEmb(self.dict_vocab[self.emb_counter].size + 1, self.config.city_emb_dim)
            self.list_emb.append(self.city_emb)
            self.emb_counter += 1

        if self.config.include_state:
            self.state_emb = BoxEmb(self.dict_vocab[self.emb_counter].size + 1, self.config.state_emb_dim)
            self.list_emb.append(self.state_emb)
            self.emb_counter += 1

        if self.config.include_country:
            self.country_emb = BoxEmb(self.dict_vocab[self.emb_counter].size + 1, self.config.country_emb_dim)
            self.list_emb.append(self.country_emb)
            self.emb_counter +=1

        if self.config.include_type:
            self.type_emb = BoxEmb(self.dict_vocab[self.emb_counter].size + 1, self.config.type_emb_dim)
