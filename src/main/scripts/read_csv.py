...
def read_relationship_csv(check=False):
    ...
    with open(data_path.RELATIONSHIP_CSV, 'r+') as f:
        # Skip first line
        f.readline()
...
def read_grid_csv(part_to_retrieve=None):
    ...
    with open(data_path.GRID_CSV, 'r+') as f:
        # Skip first line
        f.readline()
...
def read_alias_csv():
    ...
    with open(data_path.ALIAS_CSV, 'r+') as f:
        # SKip first line
        f.readline()
        ...
        for line in f.readlines():
            ...
            if split[0] in dict_id2alias_name:
                dict_id2alias_name[split[0]].append(split[1])
            else:
                dict_id2alias_name[split[0]] = [split[1]]
    return dict_id2alias_name
...
def read_type_csv():
    ...
    with open(data_path.TYPES_CSV, 'r+') as f:
        # SKip first line
        f.readline()
